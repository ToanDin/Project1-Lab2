      ...
      <View style={styles.buttons}>
        {buttons.map((button) => 
          button === '=' || button === '/' || button === '*' || button === '-' || button === '+' || button === '^2' ?
              <TouchableOpacity key={button} style={[styles.button, { backgroundColor: 'yellow' }]} onPress={() => handleInput(button)}>
                <Text style={[styles.textButton, { color: 'black', fontSize: 28 }]}>{button}</Text>
              </TouchableOpacity>
            :
          button === 'DEL' || button === 'C' ?
              <TouchableOpacity key={button} style={[styles.button, { backgroundColor: darkMode ? 'yellow' : '#ededed', minWidth: '36%' }]} onPress={() => handleInput(button)}>
